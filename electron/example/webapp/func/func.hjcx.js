...
define("func/hjcx", [
    ...
    L.Class.HJCXBase = L.Class.extend({
        ...
        initialize: function() {
            ...
            this._hjcxLayerGroup = null; //航迹图层（航迹点、航迹线、关闭按钮）
            this._hjcxlayerarr = {}; //以船舶id为key的键值对
            this._shipmarker = null; //船舶图标
        },

        addHjLayerGroup: function(poslist, curship) {
            if (!this.ictmap.map.hasLayer(this._hjcxLayerGroup)) {
            ...
            for (var j = 0, lenj = poslist.length; j < lenj; j++) {
                ...
                //航迹点
                var pt = this.createHjPoint(targetobj, id);
                allLayer.push(pt);
                ...
                //在最后一个点处创建船舶图标
                if (j === lenj - 1) {
                    targetobj = L.extend(curship, targetobj);
                    this.ictmap.realtarget.updateOneTarget(targetobj);
                    shipmarker = this.ictmap.realtarget.getShipById(id);
                }
            }
            //创建航迹线
            if (latlngs.length >= 2) {
    ...
    L.ICT.Func.add("HJCX_HJKZ", {
        ...
        Class: L.Class.HJCXBase.extend({
            ...
            sucessHandler: function(data) {
                var curship = this.hjkzCurtarget;
                this.addHjLayerGroup(data, curship);
            },

            //处理websocket返回的当前目标信息
            hjkzUpdateTarget: function(data) {
                if (!this.hjkzCurtarget || !this.ictmap.map.hasLayer(this._hjcxLayerGroup)) {
                    return;
                }
                if (data.state !== 1 || !data.msg.shipList || !data.msg.shipList.length) {
                    return;
                ...
                for (var i = 0, len = shipList.length; i < len; i++) {
                    var newObj = this.ictmap.realtarget.convertTargetObj(shipList[i]);
                    var id = newObj.id;
                    if (this._hjcxlayerarr.hasOwnProperty(id)) {
                        var shipmarker = this.ictmap.realtarget.getShipById(id);
                        if (shipmarker) {
                            var shipdata = shipmarker.options.data;
                            if (this.isChange(shipdata, newObj)) {
                                var hjply = this.createhjPly(shipdata, newObj);
                                this._hjcxlayerarr[id].push(hjply);
                                this._hjcxLayerGroup.addLayer(hjply);
                                var hjpt = this.createHjPoint(newObj, id);
                                this._hjcxlayerarr[id].push(hjpt);
