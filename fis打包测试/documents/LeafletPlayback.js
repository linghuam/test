...
L.Playback.Util = L.Class.extend({
  statics: {
    ...
    getTooltipText:function(targetobj){
        // var str = [];
        // str.push('经度:');
        // str.push(targetobj.lon);
        // str.push('<br>纬度:')
        // str.push(targetobj.lat);
        // str.push('<br>时间:');
        // str.push(targetobj.time);
        // return str.join("");
        var content = [];
        content.push('<table>');
        content.push('<tr><td>批号:</td><td>'+targetobj.ph+'</td></tr>');
        content.push('<tr><td>经度:</td><td>'+targetobj.lon+'</td></tr>');
        content.push('<tr><td>纬度:</td><td>'+targetobj.lat+'</td></tr>');
        content.push('<tr><td>时间:</td><td>'+targetobj.time+'</td></tr>');
        content.push('<tr><td>航向:</td><td>'+targetobj.dir+'</td></tr>');
        content.push('<tr><td>航艏向:</td><td>'+targetobj.heading+'</td></tr>');
        content.push('<tr><td>航速:</td><td>'+targetobj.speed+'</td></tr>');
        content.push('</table>');
    ...
    //linghuam add
    createPopup:function(targetobj){
        var options = {
            className:'ict_control_playback_popupinfo'
        };
        var content = [];
        content.push('<table>');
        content.push('<tr><td>批号:</td><td>'+targetobj.ph+'</td></tr>');
...
L.Playback.Track = L.Class.extend({
        ...
        initialize : function (map,geoJSON, options) {
            ...
            ratio;
             ...
             this._initTrackInfo(); //linghuam add
        ...
        moveMarker : function(latLng, transitionTime,timestamp) {
            if (this._marker) {
                ...
                if(transitionTime !== 0 ){
                      //绘制上一次到当前时间的历史轨迹线
                      var llgs = [[latlngold.lat,latlngold.lng],[latLng.lat,latLng.lng]];
                      var polyline = new L.Polyline(llgs,this.options.trackLineOptions);  
                      this._trackLineFeatureGroup.addLayer(polyline);
                      //绘制上一次到当前时间的历史轨迹点
                      var originPt = this._getOriginPoint(timestamp);  
                      if(originPt !== null) {
                        var circle = L.circleMarker(originPt,this.options.OriginCircleOptions);
                        this._trackPointFeatureGroup.addLayer(circle); 
                         var infoObj = this._geoJSON.properties.info[timestamp]; 
                         circle.bindTooltip(L.Playback.Util.getTooltipText(infoObj),L.Playback.Util.getTooltipOptions());
                         // circle.bindPopup(L.Playback.Util.createPopup(infoObj));
                      }  
                } else{
                      //绘制从开始到当前时间的历史轨迹线
                      this._trackLineFeatureGroup.clearLayers();
                      var pts = this._getHistoryTracks(timestamp);
                      if(pts.length > 1){
                        var polylineHis = new L.Polyline(pts,this.options.trackLineOptions);
                        this._trackLineFeatureGroup.addLayer(polylineHis); 
                      }                       
                      //绘制从开始到当前时间的历史轨迹点
                      this._trackPointFeatureGroup.clearLayers();
                      var markers = this._getOriginPoints(timestamp);
                      if(markers && markers.length>0){
                           for(var i=0;i<markers.length;i++){
                             var marker = markers[i];  
                             this._trackPointFeatureGroup.addLayer(marker);
                           }                                                  
                       } 
                }
                //linghuam add -end
        ...
        //获取已经走过的所有轨迹点
        _getHistoryTracks:function(timestamp){
            var latlngs = [];
            for(var t in this._ticks){
                if(this._ticks.hasOwnProperty(t) && t<=timestamp){
                   var lng = this._ticks[t][0];
                   var lat = this._ticks[t][1];
        ...
        //linghuam 得到已经走过的历史轨迹点（不是插值生成的原始点）
        _getOriginPoints:function(timestamp){
           var mutipoint = [];
           for(var t in this._ticks){
                if(this._ticks.hasOwnProperty(t) 
                && t<=timestamp 
                && this._geoJSON.properties.time.indexOf(parseInt(t))!== -1){

                       var lng = this._ticks[t][0];
                       var lat = this._ticks[t][1];
                       var latlng = L.latLng(lat,lng);
                       var cricleMarker = L.circleMarker(latlng,this.options.OriginCircleOptions);
                       var infoObj = this._geoJSON.properties.info[t]; 
                       cricleMarker.bindTooltip(L.Playback.Util.getTooltipText(infoObj),L.Playback.Util.getTooltipOptions());
                       // cricleMarker.bindPopup(L.Playback.Util.createPopup(infoObj));  
                       mutipoint.push(cricleMarker);
                }
            }
            return mutipoint;
        },
        
        //linghuam 获取是原始点的当前轨迹点
        _getOriginPoint:function(timestamp){
           var i = this._geoJSON.properties.time.indexOf(timestamp);
           if( i === -1) {
              return null;
           }
           else{
              var lng = this._ticks[this._geoJSON.properties.time[i]][0],
                  lat = this._ticks[this._geoJSON.properties.time[i]][1];
...
L.Playback.Clock = L.Class.extend({
  ...
  //linghuam add -start
  quickSpeed:function(){  //加速
      this._speed = this._speed/this.options.speed >= this.max_speed ? this._speed : (this._speed+this.options.speed);
      this._transitionTime = this._tickLen/this._speed;
      if(this._intervalID){
        this.stop();
        this.start();
  ...
  slowSpeed:function(){ //减速
      this._speed = this._speed/this.options.speed<= 1 ? this._speed : (this._speed-this.options.speed);
      this._transitionTime = this._tickLen/this._speed;
      if(this._intervalID){
        this.stop();
        this.start();
...
L.Playback.PlayControl = L.Control.extend({
    ...
    onAdd : function (map) {
        ...
        var self = this;
        var playback = this.playback;
        playback.setSpeed(this.playback.options.speed);
