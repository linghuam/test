...
define("func/hjcx",[   
  ...
  L.Class.HJCXBase = L.Class.extend({
		...
		createHjPoint:function(targetobj){			
			...
			//添加鼠标移上后的提示信息
			var tiptitle = this.getTiptitle(targetobj);
			var tipoptions = this.getTipOptions();
			Lmarker.bindTooltip(tiptitle,tipoptions).openTooltip();	

			return Lmarker;
		},

		getTiptitle:function(targetobj){
			//中英文
			...
			var hsxlang = $.i18n.prop('common_ship_heading');
			...
			//航艏向
			var he = targetobj.heading;
			//船速
			var sp = targetobj.speed;

            var tiptitle = '';
            tiptitle += 'MMSI:' + targetobj.mmsi + '<br>';
			tiptitle += lnglang + lon + "<br>";
			tiptitle += latlang + lat + "<br>";
			tiptitle += timelang + time + "<br>";
			tiptitle += helang + hx + "°<br>";
			tiptitle += hsxlang + he + "°<br>";
			tiptitle += splang + sp +' '+unitlang;
			return 	tiptitle;		
		},

		getTipOptions:function(){
			return {
				...
				permanent: false,
				opacity: 0.8,//透明度	
				className: "realtarget-label-target" 
			}
		},
		...
		_markerCloseClickEvt:function(e){
			...
			L.ict.app.menu.mainmenu.enableMenu();
		       ...
		       var $container = this._popPanel.getContainer().find(".hjcx_more_Container"),
		            ...
		            self.ajax.post(url,data,true,this,function(res){
		                if(res.state !== 1){
		                  ...
		                  $msg1.text($.i18n.prop('func_tshf_zdmb_error_search1'));
        					...
        					var nobj = self.ictmap.realtarget.convertshipInfoObj(res.msg.searchShip);
        					self.addTarget(nobj); 
        					self.updateList();
        					$container.find(".shipIMO").val('');
		                }
		            });      
		         ...
		         $container.find(".deleteBtn").on("click",function(event){
		          ...
		          $container.find(".hjcxBtn").on("click",function(event){
		              ...
		              var checkRes = self.dateUtil.checkStrartEndTime(startTime+":00:00",endTime+":00:00",self.config.timerange);
		              if(!checkRes.result){
		              ...
		              self.ajax.post(url,data,true,self,function(res){
		                ...
		                if(res.state != 1){
		                   ...
		                   $msg2.text($.i18n.prop('func_hjcx_error'));
		                	...
		                	L.ict.app.menu.mainmenu.disableMenu();
