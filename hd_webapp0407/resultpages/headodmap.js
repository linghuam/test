...
var shortTimeFormat = (function() {
    /*
     * 改变了页面低端播放条中时间的显示方法，从年月日到年月*/
//        var fd = d3.time.format("%d.%b.%y");
    var fd = d3.time.format("%b.%y");
    return function(ms) {
...
//            console .log(dateRange[0])
    ...
    function redrawCanvas() {
        ...
        if (setting.showEdge || selected){
            ...
            while(--l > -1) {
                ...
                /*bufCtx.lineWidth用来改变连线的宽度*/
                bufCtx.lineWidth =[1,2,3,4];
        ...
        if (setting.showChild) {
            ...
            while(--l > -1) {
                ...
                if (setting.fadingTail && setting.showTrack) {
                    ...
                    // bufCtx.lineWidth = x/80;
                    //console.log(bufCtx.lineWidth)
    ...
    /*
     * function appendextlegend显示下方直方图*/
...
//                    });
    ...
    function lme(d) {
        ...
        tooltip.html([
            ...
            d.value.now.length,
            "</b><br/>吨位: <b>",
            th(d3.sum(d3.values(d.nodeValue.supplied))),
            "M.</b><br/>"
        ].join(''));
        ...
        if (tooltip.style("display") == "none") {
            ...
            if (d.type == typeNode.parent) {
                res = [
                    ...
                    " "+ $.i18n.prop('toppanel_keysearch_type2') +": <b>",
...
//                        d.nodeValue.product,
                    "</b><br/>ShipType: <b",
                    cat == "ShipType" ? ' style="text-shadow: 0 0 2px rgba(0, 0, 0, .8);color:' + extColor(d.nodeValue[cat]) + '"' : "",
                    ">",
                    d.nodeValue.sector,
//                        "</b><hr/>Procurement: <b>",
//                        "</b><br/>• Type: <b",
...
(function() {
    ...
    ports.getByIsoId = function(id) {
        var c,
            l = ports[1].length;

        while(--l > -1) {
            c = ports[1][l];
//                console.log(c.iso2code)
            if (c.iso2code == id)
//                    console.log(c.iso2Code)
                return c;
        }
...
//                desc : d['Contract Description'],
            date : Date.parse(d['Contract_Signing_Date'])
        };
    }
...
//            d.value = d.basevalue = parseInt(d["Weight"].substring(1));
        d.value = parseInt(d["Weight"])
//            console.log(d.value)
        d.valueOf = value;

        d.borrower = initCounty(d["DestinationCode"], d["Destination"]);
        d.borrower.borrowed += d.value;
        d.supplier = initCounty(d["OriginCode"], d["Origin"]);
...
//            d.id = d['WB_Contract_Number'] + d.project.id + d.supplier.key + d.borrower.key;
        d.id =  d['WB_Contract_Number'] +  d.supplier.key + d.borrower.key;
        d.date = d.contract.date;
        d.asdate = Date.parse(d['As_of_Date']);
...
//                    "Product line"
    ...
    var year = $.i18n.prop('port_query_gxys_time');

    d3.select("#fiscalYear")
        .on("change", function(d) {
            fy = +this.value;
            if (document.location.href.indexOf('visualizing.org') > -1)
                document.location = "index.html#year=" + fy;
            btnReload.show();
        })
        .selectAll('option')
        .data([year])
    ...
    d3.json(document.location.href.indexOf('visualizing.org') > -1
         ...
         */
//            if (document.location.href.indexOf('visualizing.org') > -1)
        d3.csv("201502_new.csv", request);
//            d3.json("data/jackson.json", request);
//            else
//                d3.csv("data/"+fy + '.csv', request);
    });

})();
//    d3.csv("data/20151.csv", function(data) {console.log(data)});
//    d3.json("data/jackson.json",function(error,data2) {console.log(error, data2)});
