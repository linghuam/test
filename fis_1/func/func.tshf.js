...
//            //窗口关闭事件
//            self._areaPanel.on("popPanelRemove",function(){
...
//                //判断区域
//                if(!self._layer){
//                 $container.find(".msg").text('请绘制区域');
//                 return;
//                }
//               //判断时间
//              var startTime = $container.find(".startTime").val();
//              var endTime = $container.find(".endTime").val();                
//              var checkRes = self.checkStrartEndSj(startTime,endTime);
//              if(checkRes != "1"){
//                $container.find(".msg").text(checkRes);
//                return ;
//              }
...
//                data.rulat = bounds.getNorthEast().lat;
//                data.mode = Project_ParamConfig.CurrentMode;
//              data.startTime = self.getUnixTime(startTime);
//              data.endTime = self.getUnixTime(endTime);
//              var url = self.config.areaUrl;
//              self.ajax.post(url,data,true,self,function(res){
//                   if(res.state != 1){
//                    $container.find(".msg").text(res.msg);
//                 }else{
//                    //回放
//                        this.playBack(res);
//                 }
...
//                  return;
//             }
//             var url = self.config.getShipByBHUrl;
//             var data = {
//               num:key,
//               type:3,
//               mode:Project_ParamConfig.CurrentMode,
...
//           for(var i=0,len=items.length;i<len;i++){
//              var num = items[i].id;
//              var name = items[i].sn || items[i].id;
//              html.push('<li data-key="'+num+'"><label><input type="checkbox">'+'&nbsp&nbsp'+name+'</label></li>');
//           }
//           html.push('</ul></div>');
//           html = html.join("");
//           $listContent.html(html);
...
//           html.push('  </div>');
//           html.push('  <div class="shiplist">');
//           html.push('    <div><label><input type="checkbox" class="allchk">全选</label></div>');
//           html.push('    <hr>');
//           html.push('    <div class="listContent"></div>');
//           html.push('  </div>');
//           html.push('  <p class="msg msg2"></p>');
...
//        //验证开始时间和结束时间
//        checkStrartEndSj:function(startsj,endsj){
//           if(startsj == "" || endsj == "") return "请输入起始时间和结束时间！";
//           var startsjnew = this.dateUtil.getDateFromformatFull(startsj+':00:00');
//           var endsjnew = this.dateUtil.getDateFromformatFull(endsj+':00:00');            
//           startsjnew = startsjnew.valueOf();
//           endsjnew = endsjnew.valueOf();
//           if (startsjnew>endsjnew){
//               return "开始时间不能晚于结束时间！";
//           }else{
//               var sjc=3*30*24*60*60*1000;
