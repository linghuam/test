define("leaflet/rasterLayer",["leaflet","jquery"],function(L){
	...
	L.GridLayer.RasterLayer = L.GridLayer.extend({
        ...
        createTile: function (coords) {
            ...
            if(pts.length <= 0){
               ...
               var meanValue = this.calculateSumValue(pts);
               tile.innerHTML = meanValue;              
               tile.style.textAlign = "center";
               tile.style.lineHeight = this.options.tileSize+"px";
               tile.style.color="#000";
               tile.style.backgroundColor= this.getBgColor(meanValue);
           ...
           // 	   var latlng = L.latLng(pts[i].la,pts[i].lo);
           // 	   var cor = this.getCoorBylatlng(latlng);
           // 	   pts[i].cor = "["+cor.x+","+cor.y+"]";
           // }
           // tile.innerHTML = "";
           // tile.innerHTML += "xy:["+coords.x+","+coords.y+"]<br/>";
           // tile.innerHTML += "latlngs:"+JSON.stringify(pts)+"<br/>";
        ...
        getCoorBylatlng:function(latlng){
        	...
        	//test
        	// var point = L.point(pt.x,pt.y);
        	// var laln = this._map.layerPointToLatLng(point);
        	// if(latlng.lat == laln.lat && latlng.lng == laln.lng){
        	// 	console.log("latlng:"+latlng.toString()+";"+"laln:"+laln.toString());
        	// }
        	//test
        	return coor;
        ...
        getPointsInRaster:function(coords){
            ...
            for(var i=0,len=da.length;i<len;i++){
            	var obj = da[i];
            	// if(obj.la > maxlat) maxlat = obj.la;
            	// if(obj.la < minlat) minlat = obj.la;
            	// if(obj.lo > maxlng) maxlng = obj.lo;
            	// if(obj.lo < minlng) minlng = obj.lo;
            	var latlng = L.latLng(obj.la,obj.lo);
            	var cor = this.getCoorBylatlng(latlng);
            	if(cor.x == coords.x && cor.y == coords.y){
                    pts.push(obj);
            	}
            }
            // console.log("maxminLat:("+minlat+","+maxlat+")"+"maxminlng:("+minlng+","+maxlng+")");
            return pts;
        }
