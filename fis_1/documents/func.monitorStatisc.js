         ...
         Class: L.Class.extend({
            ...
            _secondFilterEvt:function(e){
                ...
                if(type == "realTime-0"){
                   ...
                   //发送请求，更新表格
                   var url = _this.config.realtimeCountUrl;
                   var data = {};
                   data.mode = Project_ParamConfig.CurrentMode;
                   L.Util.extend(data,_this.getArea());
                   _this.sendAjaxPoll(url,data,_this.realtimeCountRes);  

                 //目标列表
                }else if(type == "realTime-1"){
                   $container.find(".result").css("display","none").end().find(".result-realTime-1").css("display","block");        
                   //发送请求，更新表格   
                   var url = _this.config.realtimeAreaUrl;
                   var data = {};
                   data.mode = Project_ParamConfig.CurrentMode;
                   data.limit = _this.ictmap.realtarget.config.limit;
                   data.timeout =  _this.ictmap.realtarget.config.timeout;                  
                   L.Util.extend(data,_this.getArea());
                  _this.sendAjaxPoll(url,data,_this.realtimeAreaRes);  
                }
            },
            ...
            realtimeCountRes:function(data){
                ...
                var $container = this._resultPopPanel.getContainer().find(".monitotleResContainer");
                var sumCount = data.msg.areacount;
                var list = data.msg.areacountlist;
                var html = [];
                html.push('<p>实时船舶总数：'+sumCount+'艘</p>');
                html.push('<table>');
                html.push('<thead><tr><th>国别</th><th>散货船</th><th>搜救船</th><th>油轮</th><th>拖轮</th><th>渔船</th><th>拖船</th><th>客船</th><th>其他</th></tr></thead>');
                html.push('<tbody>');
            ...
            realtimeAreaRes:function(data){
                if(!this._resultPopPanel) {return;}
                var $container = this._resultPopPanel.getContainer().find(".monitotleResContainer");
                var list = data.msg.shipList;
                var html = [];
                html.push('<table>');
                html.push('<thead><tr><th>批号</th><th>模式</th><th>来源</th><th>时间</th>'+
                  '<th>MMSI</th><th>IMO</th><th>船名</th><th>呼号</th><th>国家</th><th>船舶类型</th>'+
                  '<th>航行状态</th><th>经度</th><th>纬度</th><th>航向</th><th>船艏向</th><th>转向率</th><th>航速(节)</th></tr></thead>');
                html.push('<tbody>');
                for(var i=0,len=list.length;i<len;i++){
                  var obj =  this.ictmap.realtarget.convertTargetObj(list[i]);
