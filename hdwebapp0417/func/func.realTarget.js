  ...
  "func/cdgladdship"
   ...
   L.ICT.RealTarget = L.Class.extend({
        ...
        addRasterLayer:function(){
            ...
            var url = this.config.rasterUrl;
            // var url = './documents/data.json';
            this.ajax.post(url,{timeout:this.config.timeout},true,this,function(res){
                 if(res.state !== 1){
                      console.error(res.msg.error);
                 }else{
        ...
        _markerClickEvt:function(event){
            ...
            this.ajax.post(url,data,true,this,function(res){
               if(res.state !== 1){              
                  ...
                  var targetobj = this.convertshipInfoObj(res.msg);
                  //发送请求  船舶是否被添加到船队
                  var isIncdUrl = this.config.isIncdUrl;
                  var isadd = false,self = this;
                  this.ajax.post(isIncdUrl,{mmsi:id},true,this,function(result){
                      if(result.state === 1){
                          isadd = true;
                      }
                  }).always(function(){
                      self.createShipInfoPopPanel(targetobj,isadd);
                      self._initPopEvt();  
                  });                
               }
            },function(error){
        ...
        //船舶信息弹框
        createShipInfoPopPanel:function(targetobj,isadd){
            if( this._shipinfoPopPanel){ 
              this.showShipInfoPopPanel();
              this._shipinfoPopPanel.setContent(this.getPopupContent(targetobj,isadd));
              return;
            }
            //中英文            
            ...
            var options = {
                ...
                contentHTML:this.getPopupContent(targetobj,isadd)
            };      
        ...
        convertshipInfoObj:function(oneinfo){
            ...
            var onetarget = {};
            onetarget.id =  oneinfo.mi;//oneinfo.ti; 
        ...
        //获取船舶详细信息页面
        getPopupContent:function(targetobj,isadd){
            //中英文
            ...
            var bjcd = $.i18n.prop('func_cdgl_editfleet_titlename');
            ...
            if(isadd){
               html.push('<button type="button" class="btn editcd">'+ bjcd +'</button>');
            }else {
        ...
        _initPopEvt:function(){
            ...
            $(".ict_realTarget_popupContainer .btnDiv>button").on("click",function(e){
                ...
                if(!L.ICT.Func["UserLogin"].getInstance().isLogin()){
                 ...
                 } else if($(this).hasClass('addcd')){  //添加船队
                    var  cdglshipadd=new L.ICT.CdglAddShip();
                    cdglshipadd.addShipToFleet(cdglshipadd,id,shipname,mmsi,function(){ 
                        //添加成功回调
                        if(self.currentTarget.options.data.id === id && self._shipinfoPopPanel){
                            var bjcd = $.i18n.prop('func_cdgl_editfleet_titlename');
                            var bjhtml = '<button type="button" class="btn editcd">'+ bjcd +'</button>';
                            self._shipinfoPopPanel.getContainer().find('button.addcd').replaceWith(bjhtml);
                            self._initPopEvt();
                        }
                    });
                 } else if($(this).hasClass('editcd')){ //编辑船队
                    L.ict.app.menu.mainmenu.activeMenu('ict_menu_main_cdgl');
                 }
