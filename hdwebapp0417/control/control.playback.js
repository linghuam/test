...
L.ICT.Control.PlayBack = L.Class.extend({
    ...
    options:{
       ...
       speed:1,//默认速度
       Max_Speed:Math.pow(2,20), //最大速度倍数 2的n次方
       ...
       marker:{  //marker options
        ...
        // getPopup:function(feature){
          ...
          icon: L.icon({
                iconUrl: 'themes/images/stateplayback/ship.png',
                iconSize: [12, 25],
    ...
    */
    ...
    onAdd:function(map){
        ...
        if(this.getStartTime() === this.getEndTime()){
           L.ict.app.util.dialog.alert($.i18n.prop('dialog_alert_title'),$.i18n.prop('func_tshf_error1'));
        }
    ...
    _initStyle:function(){
         ...
         $container.find(".item-start").attr("title",$.i18n.prop('control_playback_play'));
         $container.find(".item-restart").attr("title",$.i18n.prop('control_playback_again'));
         $container.find(".item-slow").attr("title",$.i18n.prop('control_playback_slowdown'));
         $container.find(".item-quick").attr("title",$.i18n.prop('control_playback_speedup'));
         $container.find(".item-close").attr("title",$.i18n.prop('control_playback_close'));
    },  
      ...
      var self = this,
       ...
       self.addCallback(function(ms){
          if(ms >= self.getEndTime()){
              ...
              $playbtn.attr("title",$.i18n.prop('control_playback_play'));
          }
    ...
    _btnClick:function(e){
        ...
        switch (type){
          ...
          if($this.hasClass("play")){
             ...
             $this.attr("title",$.i18n.prop('control_playback_pause'));
             this.start();
          }else{
             $this.addClass("play");
             $this.find("img").attr("src","themes/images/stateplayback/ico_play.png");
             $this.attr("title",$.i18n.prop('control_playback_play'));
             this.stop();
          }         
             break;
          case 3:
             ...
             $this.siblings(".item-start").attr("title",$.i18n.prop('control_playback_pause'));
             this.rePlaying();
             break;
    ...
    setTime:function(){
      ...
      this.addCallback(function(){
          curTime = L.ict.app.util.dateTime.formatDateHM(new Date(self.getTime()));
          $container.find(".playback-scrollbar .curTime").text(curTime);   
      }); 
    ...
    dataTransform:function(data){
      ...
      for(var i=0,len=data.length;i<len;i++){
         ...
         for(var j=0,lenj=data[i].posList.length;j<lenj;j++){
            ...
            obj.info_speed = parseInt(pj.sp/10) +' '+ $.i18n.prop('common_ship_speed_unit');
            dataobj.timePosList.push(obj);
            //test
            // L.marker(L.latLng(obj.lat,obj.lng)).addTo(this._map);
         }
         datas.push(dataobj);
      }
      return datas;
