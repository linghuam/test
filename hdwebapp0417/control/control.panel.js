  ...
  */
  L.ICT.DrawPopPanel = L.ICT.PopPanel.extend({
    ...
    initialize:function(layer,layerType,options){   
      ...
      switch(layerType){
        case "circle":
             ...
             options.title = $.i18n.prop('control_drawpoppanel_circle_title');
             break;
        case "rectangle":
             options.contentHTML = this.getRectContent();
             options.title = $.i18n.prop('control_drawpoppanel_rect_title');
             options.width = 420;
             break;
        case "polygon":
    ...
    getCircleContent:function(){
      ...
      var c = $.i18n.prop('control_drawpoppanel_circle_center');  
      var r = $.i18n.prop('control_drawpoppanel_circle_radius');  
      var u = $.i18n.prop('control_drawpoppanel_circle_unit');    
      var html = [];
      html.push('<div class="drawPopPanel circleContainer">');
      html.push('<ul>');
      html.push('<li><label>' + c + '</label>'+this.getItemHtml(latlng)+'</li>');
      html.push('<li><label>' + r + '</label><input type="number" class="circleRadius" value="'+radius+'">&nbsp&nbsp<label>'+ u+ '</label></li>');
      html.push('</ul>');
    ...
    getRectContent:function(){
       ...
       var lb = $.i18n.prop('control_drawpoppanel_rect_lb');
       var rt = $.i18n.prop('control_drawpoppanel_rect_rt');
       var html = [];
       html.push('<div class="drawPopPanel rectContainer">');
       html.push('<ul>');
       html.push('<li><label>'+ lb +'</label>'+this.getItemHtml(sw)+'</li>');
       html.push('<li><label>'+ rt +'</label>'+this.getItemHtml(ne)+'</li>');       
       html.push('</ul>');
       html.push(this.getBtnHTML());
       html.push('</div>');
       return html.join("");
    ...
    getBtnHTML:function(){
      var okstr = $.i18n.prop('common_btn_ok');
      var cancelstr = $.i18n.prop('common_btn_cancel');
      var html = [];
      html.push('<div class="btnDiv">');
      html.push('<button type="button" class="btn okBtn">'+ okstr +'</button>');
      html.push('<button type="button" class="btn cancelBtn">'+ cancelstr +'</button>');
      html.push('</div>');
      return html.join("");
