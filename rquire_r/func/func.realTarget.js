  ...
  "func/cdgladdship"
   ...
   L.ICT.RealTarget = L.Class.extend({
        ...
        getPopupContent:function(targetobj){
            ...
            //增加航速字段
            html.push('<tr><td>'+ sp +'</td><td colspan=3>'+targetobj.speed+' ' +jie +'</td></tr>');            
            html.push('<tr><td>'+ zxl +'</td><td>'+targetobj.rotsensor+'</td><td>'+ mdd +'</td><td>'+targetobj.destination+'</td></tr>');
        ...
        _initPopEvt:function(){
            ...
            $(".ict_realTarget_popupContainer .btnDiv>button").on("click",function(e){
                 var id = self.currentTarget.id;
                 var shipname = self.currentTarget.shipname;
                 var mmsi = self.currentTarget.mmsi;
