  ...
  L.ICT.Map = L.ICT.BaseObject.extend({
        ...
        OperateState:{
          ...
          this.ajax = new L.ICT.Ajax();
                ...
                // var center2 = this.pointChinge([options.MapOptions.center[1],options.MapOptions.center[0]]);

                //地图对象
    			var map = L.map('mappanel', {
    					center:L.latLng(center), //L.latLng(center2[1],center2[0]), // L.latLng(center),
    					zoom: zoom,
    					minZoom: minZoom,
    					maxZoom: maxZoom,
              //EPSG3857 : WGS 84 Pseudo-Mercator -- Spherical Mercator, Google Maps, OpenStreetMap, Bing, ArcGIS, ESRI
              // crs: L.CRS.EPSG3857, //google准确，海图不准
              //EPSG3395 : WGS 84 / World Mercator
              crs:L.CRS.EPSG3395, //海图准确，google不准
            ...
            new L.ICT.Control.LayerSwitch()
               ...
               .on("baseLayerChangeEvent",function(e){
                   ...
                   //    this.realtarget.googleToChart();
                   //    this.portlayer.googleToChart();
                   ...
                   //    var lat = c.lat - offsetLat;
                   //    var lng = c.lng - offsetLng;
                   //    this.map.panTo(L.latLng(lat,lng));
                   // } else if(oldlayer.options.id === 'baseLayer-ocean'){
                   //    this.realtarget.chartToGoogle();
                   //    this.portlayer.chartToGoogle();
                   //    var c = this.map.getCenter();
                   //    var offsetLat = Project_ParamConfig.MapOptions.offset.offsetLat;
                   //    var offsetLng = Project_ParamConfig.MapOptions.offset.offsetLng;
                   //    var lat = c.lat + offsetLat;
            ...
            // L.control.measureControl({position:'topleft'}).addTo(this.map);

            //地图事件
            // this.map.on("load",this._mapLoadEvt,this)
            //         .on("zoomend",this._mapZoomEndEvt,this)
            //         .on("moveend",this._mapMoveEndEvt,this)                   
            //         .on("click",this._mapClickEvt,this)
            //         .on("dblclick",this._mapdblClickEvt,this)
            //         .on("resize",this._mapResizeEvt,this)
            ...
            //地图
            // var p = [116.405640,39.901309];
            // p = this.pointChinge(p);
            // var latlng = L.latLng(p[1],p[0]);
            ...
            // }).addTo(this.map);
            // //test
            // var obj = transform(116.405640,39.901309);
        ...
        _mapLoadEvt:function(e){
            // console.log("mapload");
        },

        _mapZoomEndEvt:function(e){ //同时也会触发_mapMoveEndEvt事件
           // this.realtarget.addRealTargetLayer();
        },

        _mapMoveEndEvt:function(e){
           // if(this.hasPoppanel()) return;
           // this.realtarget.addRealTargetLayer();
           // this.portlayer.addPortLayer();
        ...
        _mapdblClickEvt:function(e){
            // console.log("dblclick");
        },

        _mapResizeEvt:function(e){
            // console.log("_mapResizeEvt");
            // $("#mappanel").css("height",$(window).height()- $("#toppanel").height());
        },
